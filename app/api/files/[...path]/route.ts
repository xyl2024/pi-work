...
// GET — delegated to ../handler.ts (shared with the zero-segment /api/files
// route that serves the filesystem root; see handler.ts for the full logic).
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ path: string[] }> }
) {
  const { path: segments } = await params;
  return handleFilesGet(request, segments);
}
