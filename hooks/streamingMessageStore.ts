...
const EMPTY_SNAPSHOT: StreamingSnapshot = { isStreaming: false, isThinking: false, streamingMessage: null };
...
function setSnapshot(e: Entry, next: StreamingSnapshot) {
  if (e.snapshot.isStreaming === next.isStreaming && e.snapshot.isThinking === next.isThinking && e.snapshot.streamingMessage === next.streamingMessage) return;
...
function flush(key: string) { const e = entry(key); e.raf = null; const message = e.pending; e.pending = null; if (message) setSnapshot(e, { isStreaming: true, isThinking: isThinkingMessage(message), streamingMessage: message }); }
...
export function startStreaming(key: string) {
  ...
  e.pending = null;
  clear(key);
  setSnapshot(e, { isStreaming: true, isThinking: false, streamingMessage: null });
}
