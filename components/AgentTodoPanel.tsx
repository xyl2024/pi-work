...
/**
 * AgentTodoPanel — a floating panel anchored to the chat input's upper-right
 * edge, surfacing the agent's live task plan.
 *
 * Position strategy:
 * - Rendered inside the input wrapper's `position: relative` container with
 *   `position: absolute`, so it follows the input width/placement exactly.
 * - Bottom-anchored above the input (`bottom: calc(100% - 2px)`), so expanding
 *   grows upward instead of downward.
 * - Hidden when there is no in-progress task and on cramped mobile widths.
 *
 ...
 * - Panel height is capped to `min(260px, 42vh)` so it stays compact above
 *   the input. The panel itself is a `flex column` with `overflow: hidden`;
 *   the task list scrolls above the bottom header, keeping the header fixed
 *   on the input border while the panel expands upward.
...
export const AgentTodoPanel = memo(function AgentTodoPanel({
  ...
  if (!enabled || empty || !firstInProgress) return null;
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{
          // Anchored to the input wrapper. Because this uses `bottom`
          // instead of `top`, expanding the panel grows it upward from the
          // input's upper-right edge.
          //
          ...
          flexDirection: "column",
          overflow: "hidden",
          padding: collapsed ? "0 6px" : "6px 6px 0",
      ...
      >
        {!collapsed && (
          ...
          // panel's maxHeight. It renders above the bottom header, so the
          // expanded panel grows upward while the header stays anchored.
