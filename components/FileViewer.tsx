...
function VideoViewer({ filePath }: { filePath: string }) {
  const { t } = useI18n();
  const toast = useToast();
  const [watching, setWatching] = useState(false);
  const [bust, setBust] = useState(0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        <span>{ext || "video"}</span>
        {resolution && <span>{resolution.w}×{resolution.h}</span>}
        {duration != null && <span>{formatDuration(duration)}</span>}
        {size != null && <span>{formatSize(size)}</span>}
        <Tooltip content={watching ? t("Live sync active") : t("Not watching")}>
        <span
      ...
      >
        <div style={{ width: "min(900px, 100%)", maxHeight: "100%" }}>
          {error && (
            <div style={{ color: "#f87171", fontSize: 13, marginBottom: 12, textAlign: "center" }}>
              {error}
            </div>
          ...
          <video
            ...
            onError={() => {
              if (!error) toast.show({ kind: "error", message: t("Failed to load file") });
              setError(t("Failed to load video"));
            }}
