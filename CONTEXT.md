...
**计划外观模式（Plan view mode）**：
计划面板的三种渲染形态：**紧凑**（分区列表 + 密集行）、**卡片**（分区列表 + 卡片）、**时间轴**（不分区，按时间从过去排到未来，收件箱钉在顶部）。选择存在 localStorage。
_Avoid_: 把它当作筛选条件（它不改变哪些计划被列出，只改变分组与排列）

**计划详情弹窗（Plan detail dialog）**：
点计划行打开的那个弹窗，备注的编辑与预览在同一处：够宽时左右分栏，窄时用弹窗内的「编辑 / 预览」切换。关闭即保存（Esc / 点遮罩 / ✕ / 换一条计划）。行上只留完成、改期、复制路径、删除，行内不再有备注编辑框，也没有单独的只读预览入口。
